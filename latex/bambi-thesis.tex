...
(CPP)
This task is fundamental to many robotic applications such as cleaning, painting, underwater operations, mine sweeping, lawn mowing, agriculture, monitoring, searching, and rescue operations. 


It is implemented the grid based cellular decomposition, which is an approximate cellular decomposition approach, for the purposes of coverage. Coverage  path is calculated using wavefront propagation starting from the goal point. The algorithm could be adapted so that it take into consideration different path feature, e.g minimum number of turns, shortest path, terrain elevation profile. An important aspect especially in \acrshort{uav} application where the limited battery life force the trajectory to be as efficient as possible.
