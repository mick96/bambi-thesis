...
The coverage path planning algorithm is implemented in the ROS node named \textit{/coverage\_path\_planner}. The graph in \autoref{fig:CPP-rqt-graph} display the nodes and topics related to the CPP procedure.
\begin{figure}[ht]
    \centering
...
\begin{description}
	\item[\textit{/mission\_controller}] It handles all the mission operations as discussed in \autoref{cha:georeferencing_the_mission_s_environment}
	\item[\textit{/terrain\_data\_provider}] It provides a service (this is why in the graph it is isolated) which allow the CPP node to request terrain data. It access to Google elevation databases using the Google elevation API. The inquiry is done in the form of an URL request through the python library \textit{urllib} \cite{urllib}. 
	\item[\textit{/kml\_generator}] It generates a KML file from the CPP generated path. This task is not needed during the mission but it is of great help in debugging and for better display the CPP output.
\end{description}
...
The resulting coverage path is a piecewise function in the local reference system made up of segment connecting the center of the cells in the order provided by the algorithm shown in \autoref{sub:coverage_path_generator}. Therefore it is not 
