...
% section state_of_the_art (end)
 ...
 The scope of the Bambi project is to design an integrated robotized system capable to autonomously handle every steps of the mission so that it can be operated also by untrained personals. This must be guaranteed even in adverse environments, especially in uneven fields as it is in upland regions. \todo{????Citare il trentino????}
 ...
 All this guidelines \todo{sinonimo per dire idee alla bass} have been taken into account during the design and implementation stages (i.e. \acrshort{ros} as framework for the on-board computer).

 \subsection{Software Architecture concept} % (fold)
